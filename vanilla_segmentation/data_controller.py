...
class SegDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Image.fromarray(label, mode='L').save('img/true_label.png')
        ...
        real_rgb = copy.deepcopy(rgb)

        rgb = np.transpose(rgb, (2, 0, 1))
        rgb = self.norm(torch.from_numpy(rgb.astype(np.float32)))
        target = torch.from_numpy(target.astype(np.int64))
        # Image.fromarray(target.numpy().astype(np.int8), mode='L').save('img/true_label.png')
        # aa = 1

        return rgb, target, real_rgb
